    ...
    * **回归问题**
       ...
       + **建立第一个CART回归树T1**
           ...
           T1模型下，训练数据集对应的输出为Y_T1。计算成本
            ...
            <a href="https://www.codecogs.com/eqnedit.php?latex=\mathbf{C}&space;=&space;\mathbf{Cost(Y\_T0,Y\_T1)}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\mathbf{C}&space;=&space;\mathbf{Cost(Y\_T0,Y\_T1)}" title="\mathbf{C} = \mathbf{Cost(Y\_T0,Y\_T1)}" /></a>
