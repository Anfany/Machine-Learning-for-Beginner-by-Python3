...
上面展示的结构仅仅是一个示例，在实际的应用中卷积层后面可以不设置激活层或者池化层，可以是多个卷积层直接相连，但是多个池化层相连的情况很罕见。最后的全连接层也可以是多个。构成模式可用如下表示：
```python
...
和不同的神经网络一样，整个过程分为两个过程，一是正向传播，二是反向传播。不在涉及每一层梯度详细的推导，只是介绍简单的操作说明。实现时可利用TensorFlow自动实现梯度的计算以及参数的更新。
   ...
   +  **卷积神经网络的反传播**
      ...
      获得误差后，就要进行反向传播，计算梯度。这里主要介绍全连接层、卷积层、池化层、激活层的反向传播，如何计算梯度以及更新参数值。
