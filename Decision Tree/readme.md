    ...
    以上面给出的示例数据为例，下面说明2种形式的变量生成回归树的方式。将数据集合定义为**D**，
     ...
     + 离散变量：以学历为例
      ...
      + 连续变量：以身高为例
          ...
          1. 将身高的所有值**去重**后按照**从小到大**的顺序排列，得到集合H=[166,173,175,180], 取**相邻两个数的中间值**得到集合MH=[169.5,174,177.5],接下来的计算就类似于离散变量的情况，挨个遍历，把数据分为不大于、大于这2个数据子集。以169.5为例，把数据集分为**Dn_169.5=D(身高<=169.5)和Dm_169.5=D(身高>169.5)**，数据子集相应的动心度组成的集合分别为**Mn_169.5,Mm_169.5**,集合相应的均值为**an_169.5, am_169.5**。
          
          对于174，177.5的符号命名规则类似，不一一赘述。
