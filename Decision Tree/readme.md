     ...
     + **离散变量：以学历为例**
        ...
        **Ddr1=D(学历=博士)** 以及 **Ddr2=D(学历!=博士)**，Ddr1中动心度构成的集合为**Mdr1**，均值为**adr1**；Ddr2中动心度构成的集合为**Mdr2**，均值为**adr2**
        ...
        + **计算示例**
           ...
           <a href="http://www.codecogs.com/eqnedit.php?latex=\\&space;{\color{Red}&space;\mathbf{Msp1=[0.56,0.52]}}\\&space;\\&space;{\color{Red}&space;\mathbf{Msp2=[0.49,0.76,0.67,0.58,0.60,0.73,0.40,0.88]}}\\&space;\\&space;{\color{Red}&space;\mathbf{asp1=0.54,&space;asp2=0.63875}}\\&space;\\&space;{\color{Red}&space;\mathbf{MSE(sp)=[(0.56-0.54)^{2}&plus;(0.52-0.54)^{2}]}}&space;\\&space;\\\cdots&space;\cdots&space;\cdots&space;\cdots&space;{\color{Red}&space;\mathbf{&plus;[(0.49-0.63875)^{2}&plus;\cdots&space;&plus;(0.88-0.63875)^{2}]}}\\&space;\\\cdots&space;\cdots&space;\cdots&space;\cdots{\color{Red}&space;\mathbf{=0.0008&plus;0.1662875=0.1670875}}\\&space;\\&space;{\color{Red}&space;\mathbf{MSE(ms)&space;=&space;0.1752083,&space;}}{\color{Red}&space;\mathbf{MSE(dr)&space;=&space;0.18245}}\\" target="_blank"><img src="http://latex.codecogs.com/gif.latex?\\&space;{\color{Red}&space;\mathbf{Msp1=[0.56,0.52]}}\\&space;\\&space;{\color{Red}&space;\mathbf{Msp2=[0.49,0.76,0.67,0.58,0.60,0.73,0.40,0.88]}}\\&space;\\&space;{\color{Red}&space;\mathbf{asp1=0.54,&space;asp2=0.63875}}\\&space;\\&space;{\color{Red}&space;\mathbf{MSE(sp)=[(0.56-0.54)^{2}&plus;(0.52-0.54)^{2}]}}&space;\\&space;\\\cdots&space;\cdots&space;\cdots&space;\cdots&space;{\color{Red}&space;\mathbf{&plus;[(0.49-0.63875)^{2}&plus;\cdots&space;&plus;(0.88-0.63875)^{2}]}}\\&space;\\\cdots&space;\cdots&space;\cdots&space;\cdots{\color{Red}&space;\mathbf{=0.0008&plus;0.1662875=0.1670875}}\\&space;\\&space;{\color{Red}&space;\mathbf{MSE(ms)&space;=&space;0.1752083,&space;}}{\color{Red}&space;\mathbf{MSE(dr)&space;=&space;0.18245}}\\" title="\\ {\color{Red} \mathbf{Msp1=[0.56,0.52]}}\\ \\ {\color{Red} \mathbf{Msp2=[0.49,0.76,0.67,0.58,0.60,0.73,0.40,0.88]}}\\ \\ {\color{Red} \mathbf{asp1=0.54, asp2=0.63875}}\\ \\ {\color{Red} \mathbf{MSE(sp)=[(0.56-0.54)^{2}+(0.52-0.54)^{2}]}} \\ \\\cdots \cdots \cdots \cdots {\color{Red} \mathbf{+[(0.49-0.63875)^{2}+\cdots +(0.88-0.63875)^{2}]}}\\ \\\cdots \cdots \cdots \cdots{\color{Red} \mathbf{=0.0008+0.1662875=0.1670875}}\\ \\ {\color{Red} \mathbf{MSE(ms) = 0.1752083, }}{\color{Red} \mathbf{MSE(dr) = 0.18245}}\\" /></a>
      ...
      + **连续变量：以身高为例**
        ...
        174，177.5的符号命名规则类似，不一一赘述。
          ...
          <a href="http://www.codecogs.com/eqnedit.php?latex=\\&space;{\color{Red}&space;\mathbf{Mn169.5=[0.58,0.60]}}\\&space;\\&space;{\color{Red}&space;\mathbf{Mm169.5=[0.56,0.49,0.76,0.67,0.73,0.40,0.52,0.88]}}\\&space;\\&space;{\color{Red}&space;\mathbf{an169.5=0.59,&space;am169.5=0.62625}}\\&space;\\&space;{\color{Red}&space;\mathbf{MSE(169.5)=0.1805873}}\\&space;\\&space;{\color{Red}&space;\mathbf{MSE(174)&space;=&space;0.133383,}}{\color{Red}&space;\mathbf{MSE(177.5)&space;=&space;0.1406857}}\\" target="_blank"><img src="http://latex.codecogs.com/gif.latex?\\&space;{\color{Red}&space;\mathbf{Mn169.5=[0.58,0.60]}}\\&space;\\&space;{\color{Red}&space;\mathbf{Mm169.5=[0.56,0.49,0.76,0.67,0.73,0.40,0.52,0.88]}}\\&space;\\&space;{\color{Red}&space;\mathbf{an169.5=0.59,&space;am169.5=0.62625}}\\&space;\\&space;{\color{Red}&space;\mathbf{MSE(169.5)=0.1805873}}\\&space;\\&space;{\color{Red}&space;\mathbf{MSE(174)&space;=&space;0.133383,}}{\color{Red}&space;\mathbf{MSE(177.5)&space;=&space;0.1406857}}\\" title="\\ {\color{Red} \mathbf{Mn169.5=[0.58,0.60]}}\\ \\ {\color{Red} \mathbf{Mm169.5=[0.56,0.49,0.76,0.67,0.73,0.40,0.52,0.88]}}\\ \\ {\color{Red} \mathbf{an169.5=0.59, am169.5=0.62625}}\\ \\ {\color{Red} \mathbf{MSE(169.5)=0.1805873}}\\ \\ {\color{Red} \mathbf{MSE(174) = 0.133383,}}{\color{Red} \mathbf{MSE(177.5) = 0.1406857}}\\" /></a>
...
+ **树的剪枝**
     ...
     其实上文的停止生长，也可看作预剪枝，但是这样设置的目的不是剪枝，而是减小树的规模，提高后面真正剪枝的计算效率。这里主要讲述比较常用的后剪枝策略：代价复杂剪枝。所谓代价复杂剪枝就是在树的规模和树的误差之间寻求一个平衡。树规模越大，树的总体误差越小。树的规模越小，树的总体误差越大。
     
     我们的目的是为了降低代价复杂度。令其为**C(T)**，其中 **T** 代表生成的树。树的规模我们用树的叶子节点的个数表示为 **|T|**，树 **T** 的总体误差记为 **E(T)**：
