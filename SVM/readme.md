   ...
   * **线性不可分情况：软间隔**
   ...
   模型多了αi<=C的限制条件，并且表达式中没有参数ξi，此时b的求值公式也会发生相应的改变。
     ...
     * **线性不可分情况：核函数**
     ...
     看前面2种情况的最终的对偶问题表达式中，都只是和内积有关。因此只要找到一个函数Ｆ，使得<img src="http://latex.codecogs.com/svg.latex?F(X_{i},X_{j})=P(X_{i})∙P(X_{j})"　border="0"/>，其中P就是非线性转换的函数。也就是F的值是非线性转换后的向量的内积。
